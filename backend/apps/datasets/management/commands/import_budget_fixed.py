...
class Command(BaseCommand):
                    ...
                    if program_key not in programs:
                        if not dry_run:
                            ...
                            program, created = Program.objects.get_or_create(
                                ...
                                defaults={
                                    'name': program_name,
                                    'description': cleaned_row['program']
                                }
                            )
    ...
    def clean_row_data(self, row):
        ...
        try:
            # 基本字段清洗
            cleaned = {
                'portfolio': self.clean_text(row.get('Portfolio', '')),
                'department': self.clean_text(row.get('Department/Agency', '')),
                'outcome': self.clean_text(row.get('Outcome', '')),
                'program': self.clean_text(row.get('Program', '')),
                'expense_type': self.clean_text(row.get('Expense type', '')),
                'appropriation_type': self.clean_text(row.get('Appropriation type', '')),
